...
class DoctorDB {
   ...
   public function getOneDoctorTelephone_us($telephone_id, $doctor_id, $address_id){
    ...
    public function get_distinct_interests(){
       ...
       public function get_available_interests($doctor_id){
        $db = Database::getDB();
        $query = "SELECT interest FROM interest_lookup WHERE interest not in (select interest from doctor_interest where doctor_id = :doctor_id)";
    ...
    public function getDoctorReportArray($state ){
        ...
        foreach ($statement as $row) {
            ...
            $doctor_list[$row['doctor_id']]['interest']=$this->getDoctorInterests($id);
  ...
  public function findDoctorInterest($doctor_id, $interest ){
    ...
    public function updateDoctorFacilityUS($address_id,$doctor_id, $status, $address_type,$address_name, $address,$address2,$address3,$address4, $city, $state, $zip5, $zip4){
...
//        $last_doctor_id = last_insert_id();
   ...
   public function setDoctorAddressInactive($address_id){
        $db = Database::getDB();
